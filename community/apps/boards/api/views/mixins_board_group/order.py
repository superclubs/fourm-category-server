...
from community.apps.boards.api.serializers import (
    ...
    BoardGroupOrderUpdateSerializer,
)
    ...
    def board_group_order(self, request, pk=None):
        ...
        if board_group.community.user == request.user:
            ...
            if board_group_order > request_order:
                board_groups.filter(order__gte=request_order, order__lte=board_group_order).update(order=F("order") + 1)
            if board_group_order < request_order:
                board_groups.filter(order__gte=board_group_order, order__lte=request_order).update(order=F("order") - 1)
