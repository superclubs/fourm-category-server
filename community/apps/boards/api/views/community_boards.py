...
# Main Section
class CommunityBoardsViewSet(mixins.ListModelMixin, GenericViewSet):
    serializers = {
    ...
    @swagger_auto_schema(
        **swagger_decorator(tag="02. 커뮤니티", id="보드 리스트 조회", description="", response={200: BoardListSerializer})
    )
    def list(self, request, *args, **kwargs):
        return super().list(self, request, *args, **kwargs)


class CommunityBoardsAdminViewSet(mixins.ListModelMixin, GenericViewSet):
    serializers = {
        "default": BoardListSerializer,
    }
    filter_backends = (DjangoFilterBackend,)
    ...
    @swagger_auto_schema(
        **swagger_decorator(tag="02. 커뮤니티 - 어드민", id="보드 리스트 조회", description="", response={200: BoardListSerializer})
    )
    def list(self, request, *args, **kwargs):
        return super().list(self, request, *args, **kwargs)
