...
class BoardOrderViewMixin:
    ...
    def board_order(self, request, pk=None):
        ...
        if board.board_group.community.user == request.user:
            ...
            board_groups = Board.objects.filter(
                board_group__community__id=board.board_group.community.id, board_group=board.board_group
            )
            request_board_groups = Board.objects.filter(
                board_group__community__id=board.board_group.community.id, board_group=request_board_group
            )

            # 보드 그룹이 같을 때
            if board.board_group == request_board_group:
                if board_order > request_order:
                    board_groups.filter(order__gte=request_order, order__lte=board_order).update(order=F("order") + 1)
                if board_order < request_order:
                    board_groups.filter(order__gte=board_order, order__lte=request_order).update(order=F("order") - 1)
            # 보드 그룹이 다를 때
            else:
