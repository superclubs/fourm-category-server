...
from community.apps.boards.api.serializers import (
    BoardCreateAdminSerializer,
    BoardGroupCreateSerializer,
    BoardGroupRetrieveSerializer,
    BoardListSerializer,
    BoardRetrieveSerializer,
    BoardUpdateAdminSerializer,
)

# Filters
from community.apps.boards.api.views.filters.index import BoardFilter
from community.apps.boards.api.views.mixins_board import (
    BoardMergeViewMixin,
    BoardOrderViewMixin,
)

# Mixins
from community.apps.boards.api.views.mixins_board_group import (
...
# Main Section
class BoardGroupViewSet(mixins.RetrieveModelMixin, GenericViewSet):
    serializers = {
    ...
    @swagger_auto_schema(
        **swagger_decorator(
            tag="03. 보드 그룹", id="보드 그룹 객체 조회", description="", response={200: BoardGroupRetrieveSerializer}
        )
    )
...
class BoardGroupAdminViewSet(
    ...
    @swagger_auto_schema(
        **swagger_decorator(
            tag="03. 보드 그룹 - 어드민", id="보드 그룹 객체 삭제", description="", response={200: BoardGroupRetrieveSerializer}
        )
    )
...
class BoardsViewSet(mixins.ListModelMixin, GenericViewSet):
    serializers = {
...
class BoardViewSet(mixins.RetrieveModelMixin, GenericViewSet):
    serializers = {
    ...
    @swagger_auto_schema(
        **swagger_decorator(tag="03. 보드", id="보드 객체 조회", description="", response={200: BoardRetrieveSerializer})
    )
    def retrieve(self, request, *args, **kwargs):
        return super().retrieve(self, request, *args, **kwargs)


class BoardAdminViewSet(
    mixins.UpdateModelMixin, mixins.DestroyModelMixin, BoardMergeViewMixin, BoardOrderViewMixin, GenericViewSet
):
    serializers = {
    ...
    @swagger_auto_schema(
        **swagger_decorator(tag="03. 보드 - 어드민", id="보드 삭제", description="", response={200: BoardRetrieveSerializer})
    )
    def destroy(self, request, *args, **kwargs):
        instance = self.get_object()
        if instance.posts.exists():
