...
# Serializers
from community.apps.reports.api.serializers import (
    ReportChoiceListSerializer,
    ReportChoiceUpdateSerializer,
)
...
# Main Section
class CommunityReportChoiceAdminViewSet(mixins.UpdateModelMixin, mixins.DestroyModelMixin, GenericViewSet):
    serializers = {
    ...
    @swagger_auto_schema(
        **swagger_decorator(tag="13. 신고 - 어드민", id="신고 사유 삭제", description="", response={204: "no content"})
    )
...
class CommunityReportChoicesViewSet(mixins.ListModelMixin, GenericViewSet):
    serializers = {
        "default": ReportChoiceListSerializer,
    }
    filter_backends = (DjangoFilterBackend,)
    ...
    @swagger_auto_schema(
        **swagger_decorator(
            tag="02. 커뮤니티", id="신고 사유 리스트 조회", description="", response={200: ReportChoiceListSerializer}
        )
    )
