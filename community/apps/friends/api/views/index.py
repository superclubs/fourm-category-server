...
class FriendViewSet(mixins.CreateModelMixin, mixins.DestroyModelMixin, GenericViewSet):
    ...
    @swagger_auto_schema(**swagger_decorator(tag="05. 친구", id="친구 삭제", description="", response={204: "no content"}))
    def destroy(self, request, *args, **kwargs):
        return super().destroy(self, request, *args, **kwargs)

    @swagger_auto_schema(
        **swagger_decorator(
            tag="05. 친구", id="친구 생성", description="", request=FriendSyncSerializer, response={200: FriendSyncSerializer}
        )
    )
    @action(detail=False, methods=["post"])
    def sync(self, request):
