...
from community.apps.posts.models.mixins.image import (
    get_thumbnail_media_url_and_medias_data,
)
...
class PostUpdateSerializer(ModelSerializer):
    ...
    communities = serializers.ListField(
        child=serializers.PrimaryKeyRelatedField(queryset=Community.objects.all()), required=False
    )
    ...
    def update(self, instance, validated_data):
        ...
        if content:
            ...
            if not instance.is_vote:
                thumbnail_media_url, medias_data = get_thumbnail_media_url_and_medias_data(
                    content=content, uuid=instance.user.uuid
                )
