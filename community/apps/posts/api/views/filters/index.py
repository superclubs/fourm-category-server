...
class PostFilter(django_filters.FilterSet):
    ...
    def profile_liked_filter(self, queryset, title, value):
        profile = Profile.available.filter(id=value, is_joined=True).first()
        if profile:
            post_ids = PostLike.available.filter(profile=profile, is_active=True).values_list('post_id', flat=True)
        return queryset.filter(id__in=post_ids)

    def profile_commented_filter(self, queryset, title, value):
        profile = Profile.available.filter(id=value, is_joined=True).first()
        if profile:
            post_ids = Comment.available.filter(profile=profile, is_active=True).values_list('post_id', flat=True)
        return queryset.filter(id__in=post_ids)
