...
class RankingGroupManager(Manager):
    ...
    def get_prev_ranking_group(self, ranking_group):
        ...
        def get_date_ago(weeks=0, days=0, hours=0, minutes=0):
            return (
                now() - timedelta(weeks=weeks, days=days, hours=hours, minutes=minutes) + timedelta(minutes=30)
            )  # 생성 시간 고려
        ...
        return (
            self.get_queryset()
            .filter(
                model_type=ranking_group.model_type, ranking_type=ranking_group.ranking_type, created__lte=prev_date
            )
            .first()
        )
