...
class CommunityLikeModelMixin(models.Model):
    ...
    def update_posts_dislike_count(self):
        self.posts_dislike_count = self.posts.filter(is_active=True, is_temporary=False).aggregate(
            Sum("dislike_count")
        )["dislike_count__sum"]

    # post 별로 comment like 갯수도 가지고 있어야 한다.
    def update_comments_like_count(self):
        self.comments_like_count = self.posts.filter(is_active=True).aggregate(Sum("comments_like_count"))[
            "comments_like_count__sum"
        ]

    def update_comments_dislike_count(self):
        self.comments_dislike_count = self.posts.filter(is_active=True).aggregate(Sum("comments_dislike_count"))[
            "comments_dislike_count__sum"
        ]
