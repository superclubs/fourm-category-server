...
class CommunitySyncViewMixin:
    @swagger_auto_schema(
        **swagger_decorator(tag="02. 커뮤니티", id="커뮤니티 싱크", description="", response={200: CommunitySyncSerializer})
    )
    ...
    def community_sync(self, request):
        ...
        return Response(
            status=status.HTTP_200_OK, code=200, message="ok", data=CommunitySyncSerializer(queryset, many=True).data
        )
